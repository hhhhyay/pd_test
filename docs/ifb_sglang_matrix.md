...
For cache-hit debugging, use a few more prompts and disable smoke warmup so
the server log excerpt shows only the tested requests:
...
If the target node is reachable only after logging in to a gateway node, use
the nested SSH mode:

```bash
python benchmark/run_ifb_matrix.py --config configs/ifb_matrix.yaml --ssh-target guobj@10.16.1.9 --nested-ssh-target nmz22 --docker-container <container-name> --client-host <server-ip> --smoke --smoke-all-hit-rates --reuse-server --no-resume
```
